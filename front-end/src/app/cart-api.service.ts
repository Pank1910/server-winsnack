...
import { CartItem } from '../interface/Cart'; // Điều chỉnh đường dẫn nếu cần

@Injectable({
  providedIn: 'root',
})
export class CartAPIService {
  private apiUrl = '/cart'; // Thay bằng URL thực tế của backend nếu cần
  ...
  updateCartItems(items: { productId: string; quantity: number; unit_price: number }[]): Observable<{ message: string }> {
    return this.http
      .put<{ message: string }>(`${this.apiUrl}/update-all`, items, {
        headers: this.getHeaders(),
        withCredentials: true,
      })
      .pipe(catchError(this.handleError));
